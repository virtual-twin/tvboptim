...
`LinearCoupling` now uses the optimized incoming-only path directly. `FastLinearCoupling` is a deprecated compatibility wrapper that maps its historical `local=...` spelling to `LinearCoupling(source=...)`; new code should use `LinearCoupling`.

...
**`tbptt_window` — truncated backpropagation (gradient horizon).** Sever the carry
...
```python
solver = Heun(tbptt_window=2000)                  # bounded gradient horizon
solver = Heun(tbptt_window=2000, block_size=200)  # horizon + bounded memory
```

...
The heterogeneous-network API is experimental. It supports a fixed partition
of one square graph, one time step, and the native Euler, Heun, and Runge–Kutta
solvers.
:::
...
We prepare the network once, then simulate, sweep it with `Space`, and optimize
it through automatic differentiation.
...
Each group keeps its natural state width: cortex `[1, 6]`, relay `[6, 2]`, and
driven `[2, 1]`. Only the routed signal is temporarily packed as `[1, 9]`.
...
variable rows and node axis. The unequal panel counts make the different state
dimensions visible immediately:
...
Cartesian product of three delayed gains and two cortical recurrence values;
...
All six simulations share static group indices, route callables, and buffer
capacity; only the selected numerical leaves are batched.
...
The curve should rise as separately lowered vector fields and signal packing
fragment the traced program, although each route still performs one transport.
...
Current support is limited to native Euler, Heun, and Runge–Kutta solvers.
Diffrax, multiple clocks, changing membership, partial graph partitions, and
networks on different node spaces are future work.

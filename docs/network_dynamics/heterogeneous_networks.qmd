...
`plot(group=...)` selects a group's native variables and local node axis. Here
the two relay nodes, initialized with a small offset, are overlaid for three
Jansen–Rit states:

```{python}
#| label: fig-heterogeneous-relay-trajectories
#| fig-cap: "Relay-group trajectories. Heterogeneous results retain each group's native variables and node axis."

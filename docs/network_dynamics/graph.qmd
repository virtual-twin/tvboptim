...
random_graph = DenseGraph.random(
    n_nodes=20,
    density=0.5,            # 50% of connections present
...
For very large networks with extremely high sparsity (< 1% density), sparse storage can reduce memory usage. TVB-Optim uses JAX's [BCOO (Batched COO) format](https://jax.readthedocs.io/en/latest/jax.experimental.sparse.html) for sparse graphs. Such high sparsity is typical in surface-based simulations with tens of thousands of vertices.
...
## Sparse edge order and aligned metadata

Sparse coupling executes directly on the stored edge list. The public
`edge_indices` property returns an `[E, 2]` array in `(target, source)` order,
and `gather_edges()` converts a dense graph-shaped value into that same order:

```python
edge_values = sparse_graph.gather_edges(graph_shaped_values)
target_e = sparse_graph.edge_indices[:, 0]
source_e = sparse_graph.edge_indices[:, 1]
```

Use the `[E]` representation for edge-shaped coupling parameters on large
sparse networks; a graph-shaped parameter remains supported as a migration
layout but still costs O(N²) storage.

`SparseDelayGraph` stores weight and delay data on one shared edge order.
Weighted edges with a zero delay retain an explicit delay slot, so delay
sweeps and gradients do not change topology. Numerical `.data` leaves may be
replaced in an already prepared configuration, while changing the number,
order, or coordinates of edges invalidates the prepared topology and requires
reconstructing the graph and calling `prepare()` again.

...
#| output: false
#| code-fold: true
#| code-summary: "Define the zero-initialized MLP helper"
...
missing term depending only on `V`. This is compatible with an accurate
trajectory fit because the observed states occupy a narrow, correlated part of
the phase plane. Recovering the intended mechanism would require stronger
information, such as restricting the correction's inputs, sampling more of the
phase plane, or regularizing unwanted state dependence.
...
single_correction = EquinoxParameter(
    make_zero_initialized_mlp(segment_module_key, width_size=SELECTED_WIDTH)
)

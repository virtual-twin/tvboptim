...
class EIBLinearCoupling(InstantaneousCoupling):
    ...
    def pre(
        ...
        S_e = incoming_states[0]  # already aligned to nodes or prepared edges
        # EDGE_PARAMS makes both operands follow the active message layout.
        c_lre = S_e * params.wLRE
        c_ffi = S_e * params.wFFI

        # Stack only the leading output-channel dimension.
...
coupling = EIBLinearCoupling(source=["S_e"])
...
# For a large SparseGraph, keep these parameters O(E) instead:
# coupling.params.wLRE = graph.gather_edges(wLRE_matrix)
# coupling.params.wFFI = graph.gather_edges(wFFI_matrix)
...
def run_fic_tuning():
    ...
    for i in range(n_fic_steps):
        ...
        # Update noise realization: swap the in-scan PRNG key.
        key, subkey = jax.random.split(key, 2)
        state_fic.noise.key = subkey
...
def run_eib_tuning():
    ...
    for i in range(n_eib_steps):
        ...
        # 6. Update noise realization: swap the in-scan PRNG key.
        key, subkey = jax.random.split(key, 2)
        state_ei.noise.key = subkey

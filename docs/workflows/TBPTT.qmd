...
subtitle: "Why long-rollout gradients break, and how the `tbptt_window` knob fixes them"
format:
  html:
    code-fold: false
    toc: true
...
backpropagation through time (TBPTT), a single solver knob `tbptt_window` that
...
single solver knob, `tbptt_window`, on the native solvers.
...
`tbptt_window` steps. The loss depends on every window, so **every window is
...
- **TBPTT** replaces the backward pass with a windowed solver
  `Heun(tbptt_window=W)`, assigning credit only over the last $W$ steps.
...
def compute_optim():
    ...
    solve_w, cfg_w = prepare(network, Heun(tbptt_window=W_TBPTT), t1=FC_T1, dt=DT)
...
`tbptt_window` only changes the backward pass. The forward trajectory, and hence
...
`tbptt_window` sets the gradient horizon, not the cost. Bounding runtime and
memory is the job of `block_size` (gradient checkpointing), an orthogonal knob
that rematerializes activations within each window and nests with `tbptt_window`.

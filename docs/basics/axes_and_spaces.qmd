...
The Axes and Spaces system is a generic parameter exploration framework for JAX-based models. It operates on [JAX pytrees](https://jax.readthedocs.io/en/latest/pytrees.html), including plain dicts, lists, dataclasses, NamedTuples, and the config objects returned by `prepare`. Replace any leaf with an axis and pass the container to `Space` to obtain fully resolved parameter combinations.
...
# Any pytree works; here a plain dict has two axes and one fixed value
preview_state = {
...
GridAxis samples a deterministic grid across a parameter range. It returns the same values on every run.
...
GridAxis and UniformAxis support a `shape` parameter. Each sample becomes a constant array of that shape, which is useful for parameters shared across brain regions:
...
Space takes a state tree with axes at varying leaves and plain values at fixed leaves, then generates the requested parameter combinations.
...
The axis and space system works with any pure JAX function. The examples use a brain network model, a common application in TVB-Optim.
...
network = Network(
    ...
    coupling={'instant': LinearCoupling(source='S', G=0.5)},
...
`prepare` returns a compiled solve function and a config object. Axes attach directly to config leaves, while `solve_fn` remains constant across the sweep.
...
## Optimising Inside a Sweep

An axis varies a state slot across lanes, while a `Parameter` marks a slot as differentiable. Multi-start optimisation needs both: each lane selects an initial value and then optimises it.

`wrap=` bridges the two. It is applied to the axis' value for each combination as it is substituted back into the state, so the model receives a `Parameter` and can optimise it directly:
...
# Eight starting points for the same parameter, optimised in parallel
starts = jnp.linspace(0.15, 0.9, 8)

multistart = {
    'G': DataAxis(starts, wrap=partial(SigmoidBoundedParameter, low=0.05, high=1.0)),
    'target': 0.6,
}

def fit(state):
    def loss(s):
        return (s['G'].constrained_value - s['target']) ** 2

    opt = OptaxOptimizer(loss=loss, optimizer=optax.adam(0.05))
    final, _ = opt.run(state, max_steps=100)
    return final['G'].constrained_value

space = Space(multistart, mode='zip')
fitted = ParallelExecution(fit, space, n_vmap=4, n_pmap=1).run()
print(f"Recovered from {len(starts)} starts: {jnp.ravel(jnp.asarray(fitted.results))}")
```

`fit` contains no parameter construction. The axis defines the wrapper and its arguments once.

Axis sampling bounds and parameter constraints are independent concepts, even when they happen to use the same interval. Bounded wrappers therefore require explicit values through `functools.partial`. A misconfigured wrapper raises when its first selected value is materialised.

Axes without `wrap` substitute raw arrays. Wrapping occurs per combination, so it does not affect `size`, `Space.N`, grouping, or `to_dataframe`.

Wrapping is a per-combination operation. A vector-valued `Parameter` is not
generally equivalent to a batch of scalar parameters, particularly when its
transform depends on the initial value. Therefore,
`Space.collect(combine=True)` rejects wrapped axes. Use `ParallelExecution` or
iteration for materialised states, and `collect(combine=False)` for raw batched
axis arrays.

Custom wrappers must be pure, trace-compatible callables that accept one
selected value and return the same JAX pytree structure for every value.
Built-in parameter types with strict input domains validate generated axis
values before mapped execution begins.

::: {.callout-tip}
`OptaxOptimizer.run` detects tracing and represents its step loop with `lax.scan`, avoiding graph unrolling with `max_steps` inside `ParallelExecution`. Per-step callbacks are unavailable under tracing; see [Parameters and Optimization](parameters_and_optimization.qmd).
:::

## Performance Comparison

For inexpensive models, JIT compilation and dispatch can cost more than parallel execution saves. The crossover depends on model complexity and problem size.
...
For a stochastic `Network` integrated with a native solver, `config.noise.key` controls the per-step noise increments. Because the key is a regular leaf in the prepared config, an axis can replace it directly. The axis system substitutes the per-point key before each call.
...
- **Common random numbers across parameter sweeps.** Place the `DataAxis` of keys on its own to share one key across all `G` values, or use `mode='zip'` to pair keys and parameter values. Reusing noise trajectories can reduce variance in comparisons between parameter settings.

For workflows that require addressable increments, such as inference over Brownian increments or deterministic replay, set `config._internal.noise_samples` to an array with shape `[n_steps, n_noise_states, n_nodes]`. The scan selects this field at trace time instead of sampling internally. Changing between `None` and an array triggers JIT retracing. This input is supported only by native solvers. Diffrax supports changing `config.noise.key`, but its `VirtualBrownianTree` does not consume pre-sampled arrays.

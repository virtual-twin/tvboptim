---
title: "Parameters and Optimization in TVBOptim"
format:
  html:
    code-fold: false
    toc: true
...
As a simple example on how to use parameters we try to find the fixed point of the reduced Wong-Wang model. This example demonstrates the key concepts of parameter optimization in TVBOptim by finding parameter values where the neural activity reaches equilibrium.
...
```{python}
from tvbo.knowledge import Model

m = Model("ReducedWongWang")
dfun = m.execute("jax")
print(m.render_code("jax"))
m.parameter_table()
```
...
def loss_fixed_point(p, S_0 = [0.8], coupling = [0.0]):
    return jnp.abs(jnp.sum(dfun(S_0, coupling, p)))
...
m.metadata.parameters["gamma"].value = p_opt.gamma
m.metadata.parameters["tau_s"].value = p_opt.tau_s
ts_opt = m.run()

# Run simulation with optimized normalized parameters
p_opt_norm = collect_parameters(p_opt_norm)
m.metadata.parameters["gamma"].value = p_opt_norm.gamma
m.metadata.parameters["tau_s"].value = p_opt_norm.tau_s
ts_opt_norm = m.run()
```
...
# Plot target line
ax.hlines(0.8, 0, 10000, color="black", linewidth=2.5, label="Target Fixed Point (0.8)")

# Plot time series
ax.plot(ts_default.data[:, 0, :, 0], label="Default Parameters", alpha=0.8)
ax.plot(ts_opt.data[:, 0, :, 0], color='red', label="Regular Parameters", alpha=0.8)
ax.plot(ts_opt_norm.data[:, 0, :, 0], color='green', linestyle='--',

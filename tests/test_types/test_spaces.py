...
from tvboptim.types.spaces import DataAxis, GridAxis, Space, UniformAxis
...
class TestSpaceBasics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nested_state = {
            "parameters": {
                "coupling": {"a": GridAxis(0.0, 1.0, 2), "b": 1.5},  # Fixed value
                "model": {"J_N": UniformAxis(-1.0, 1.0, 3)},
            },
            "config": {"dt": 0.01, "T": 1000},
        }
...
class TestSpaceIteration(unittest.TestCase):
    ...
    def test_iteration_product_mode(self):
        ...
        small_state = {"a": GridAxis(0.0, 1.0, 2), "b": GridAxis(0.0, 1.0, 2)}
    ...
    def test_iterator_protocol(self):
        ...
        self.assertNotEqual(second["param_a"], third["param_a"])
        self.assertNotEqual(first["param_a"], third["param_a"])

    def test_iteration_with_random_key(self):
        """Test iteration with specified random key."""
        state_with_random = {"uniform": UniformAxis(0.0, 1.0, 3), "fixed": 100}
    ...
    def test_space_with_numpyro_shapes(self):
        ...
        state = {
            "multivar": NumPyroAxis(
                dist.MultivariateNormal(jnp.zeros(2), jnp.eye(2)), n=3
            )
        }
...
class TestSpaceEdgeCases(unittest.TestCase):
    ...
    def test_space_deeply_nested(self):
        ...
        state = {
            "level1": {"level2": {"level3": {"param": GridAxis(0.0, 1.0, 2)}}},
            "top_level": "constant",
        }

...
from tvboptim.types import Parameter, Space, DataAxis, collect_parameters
from tvboptim.execution import SequentialExecution, ParallelExecution, SequentialResult, ParallelResult
...
class TestExecution(unittest.TestCase):
    ...
    def test_result_slicing(self):
        ...
        for i in range(5):
            ...
            self.assertAlmostEqual(float(s_slice[i][0]), float(p2_slice[i][0]), places=5)
            self.assertAlmostEqual(float(s_slice[i][1]), float(p2_slice[i][1]), places=5)
    ...
    def test_execution_with_different_functions(self):
        ...
        for i in range(5):
            self.assertAlmostEqual(float(result_sum_s[i][0]), float(result_sum_p[i][0]), places=5)
            self.assertAlmostEqual(float(result_prod_s[i][0]), float(result_prod_p[i][0]), places=5)

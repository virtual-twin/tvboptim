...
def format_pytree_as_string(pytree, name: str = "root", prefix: str = "", is_last: bool = False, 
                      show_numerical_only: bool = False, is_root: bool = True, hide_none: bool = False, show_array_values: bool = False):
    """
    ...
    try:
        ...
        if isinstance(pytree, dict):
            ...
            for i, (key, value) in enumerate(items):
                child_result = format_pytree_as_string(value, str(key), next_prefix, i == len(items) - 1, 
                                              show_numerical_only, False, hide_none, show_array_values)
        ...
        # If it's a dataclass or a custom class with __dict__ attribute
            ...
            for i, (key, value) in enumerate(items):
                child_result = format_pytree_as_string(value, key, next_prefix, i == len(items) - 1, 
                                              show_numerical_only, False, hide_none, show_array_values)
                if child_result:
                    result.append(child_result)
        ...
        # If it's a sequence (like list or tuple)
            ...
            for i, item in enumerate(pytree):
                child_result = format_pytree_as_string(item, f"[{i}]", next_prefix, i == len(pytree) - 1, 
                                              show_numerical_only, False, hide_none, show_array_values)
                if child_result:
                    result.append(child_result)
...
def pretty_print_pytree(pytree, name: str = "root", prefix: str = "", show_numerical_only: bool = False, hide_none: bool = False):
    """
...
def safe_reshape(arr, new_shape, fill_value=jnp.nan):
    ...
    if new_size < old_size:
        raise ValueError(f"New shape {new_shape} has fewer elements ({new_size}) "

...
class AbstractCallback:
    def __init__(self, every=1) -> None:
        self.every = every
    def __call__(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        if i % self.every == 0:
            return self.do(i, diff_state, static_state, fitting_data, aux_data, loss_value, grads)
...
class SavingCallback(AbstractCallback):
    ...
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        ...
        fitting_data[self.key].loc[len(fitting_data[self.key])] = [i, self.save_fun(i, diff_state, static_state, fitting_data, aux_data, loss_value, grads)] 
        return False, diff_state, static_state

class SavingLossCallback(SavingCallback):
    def __init__(self, every=1, *args: None) -> None:
        super().__init__(every, key="loss", save_fun=lambda i, diff_state, static_state, fitting_data, aux_data, loss_value, grads: loss_value)

class SavingParametersCallback(SavingCallback):
    def __init__(self, every=1, *args: None) -> None:
        super().__init__(every, key="parameters", save_fun=lambda i, diff_state, static_state, fitting_data, aux_data, loss_value, grads: diff_state)
...
class TimingCallback(AbstractCallback):
    ...
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        ...
        if self.t0 is None or i == 0:
            self.t0 = time.time()
        fitting_data[self.key].loc[len(fitting_data[self.key])] = [i, time.time() - self.t0] 
        return False, diff_state, static_state
...
class StopTimeCallback(AbstractCallback):
    ...
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        ...
        if (time.time() - self.t0) > self.time_limit:
            print(f"Stopped at step {i} after reaching time limit of {self.time_limit} seconds")
...
class PrintParameterCallback(AbstractCallback):
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        ...
        print(format_pytree_as_string(diff_state, hide_none=True, name="Parameters", show_array_values=True))
        return False, diff_state, static_state
...
class PrintGradsCallback(AbstractCallback):
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        ...
        print(format_pytree_as_string(grads, hide_none=True, name="GradientState", show_array_values=True))
...
class MultiCallback(AbstractCallback):
    ...
    def do(self, i, diff_state, static_state, fitting_data, aux_data, loss_value, grads):
        for callback in self.callbacks:
            test, diff_state, static_state = callback(i, diff_state, static_state, fitting_data, aux_data, loss_value, grads)
            if test:
                return True, diff_state, static_state
        return False, diff_state, static_state
